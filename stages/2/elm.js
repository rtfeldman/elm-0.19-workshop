...
Elm.ElmHub.make = function (_elm) {
   ...
   var _op = {};
   var isKeepable = F2(function (action,_p0) {    var _p1 = _p0;return !_U.eq(_p1.id,action.payload);});
   var update = F2(function (action,model) {
      if (_U.eq(action.actionType,"HIDE_BY_ID")) {
            var newResults = A2($List.filter,isKeepable(action),model.results);
            var newModel = _U.update(model,{results: newResults});
            return newModel;
         } else return model;
   });
   var Action = F2(function (a,b) {    return {actionType: a,payload: b};});
   var viewSearchResult = F2(function (address,result) {
      ...
      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("star-count")]),_U.list([$Html.text($Basics.toString(result.stars))]))
              ...
              ,A2($Html.button,
              _U.list([$Html$Attributes.$class("hide-result"),A2($Html$Events.onClick,address,{actionType: "HIDE_BY_ID",payload: result.id})]),
              _U.list([$Html.text("X")]))]));
   });
   ...
   var main = $StartApp$Simple.start({view: view,update: update,model: initialModel});
