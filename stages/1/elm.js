...
Elm.ElmHub.make = function (_elm) {
   ...
   var viewSearchResult = function (result) {
      return A2($Html.li,
      _U.list([]),
      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("star-count")]),_U.list([$Html.text($Basics.toString(result.stars))]))
              ,A2($Html.a,
              _U.list([$Html$Attributes.$class("result-name")
                      ,$Html$Attributes.href(A2($Basics._op["++"],"https://github.com/",result.name))
                      ,$Html$Attributes.target("_blank")]),
              _U.list([$Html.text(result.name)]))]));
   };
   var myViewFunction = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("content")]),
      _U.list([A2($Html.header,
              _U.list([]),
              _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Hello World!")]))
                      ,A2($Html.span,_U.list([$Html$Attributes.$class("tagline")]),_U.list([$Html.text("“Like GitHub, but for Elm things.”")]))]))
              ,A2($Html.ul,_U.list([$Html$Attributes.$class("results")]),A2($List.map,viewSearchResult,model.results))]));
   });
   ...
   var main = $StartApp$Simple.start({view: myViewFunction,update: update,model: initialModel});
   return _elm.ElmHub.values = {_op: _op
                               ,main: main
                               ,Model: Model
                               ,SearchResult: SearchResult
                               ,initialModel: initialModel
                               ,myViewFunction: myViewFunction
