...
Elm.ElmHub.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var SetResults = function (a) {    return {ctor: "SetResults",_0: a};};
   var HideById = function (a) {    return {ctor: "HideById",_0: a};};
   var SetQuery = function (a) {    return {ctor: "SetQuery",_0: a};};
   var Search = {ctor: "Search"};
   var viewSearchResult = F2(function (address,result) {
      return A2($Html.li,
      _U.list([]),
      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("star-count")]),_U.list([$Html.text($Basics.toString(result.stars))]))
              ,A2($Html.a,
              _U.list([$Html$Attributes.href(A2($Basics._op["++"],"https://github.com/",result.name))
                      ,$Html$Attributes.$class("result-name")
                      ,$Html$Attributes.target("_blank")]),
              _U.list([$Html.text(result.name)]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.$class("hide-result"),A2($Html$Events.onClick,address,HideById(result.id))]),
              _U.list([$Html.text("X")]))]));
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.header,
              _U.list([]),
              _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("ElmHub")]))
                      ,A2($Html.span,_U.list([$Html$Attributes.$class("tagline")]),_U.list([$Html.text("“Like GitHub, but for Elm things.”")]))]))
              ,A2($Html.input,_U.list([$Html$Attributes.$class("search-query"),A2(onInput,address,SetQuery),defaultValue(model.query)]),_U.list([]))
              ,A2($Html.button,_U.list([$Html$Attributes.$class("search-button"),A2($Html$Events.onClick,address,Search)]),_U.list([$Html.text("Search")]))
              ,A2($Html.ul,_U.list([$Html$Attributes.$class("results")]),A2($List.map,viewSearchResult(address),model.results))]));
   });
   var initialModel = {query: "tutorial",results: _U.list([])};
   var SearchResult = F3(function (a,b,c) {    return {id: a,name: b,stars: c};});
   var Model = F2(function (a,b) {    return {query: a,results: b};});
   var searchResultDecoder = A4($Json$Decode.object3,
   SearchResult,
   A2($Json$Decode._op[":="],"TODO what field goes here?",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"TODO what field goes here?",$Json$Decode.string),
   A2($Json$Decode._op[":="],"TODO what field goes here?",$Json$Decode.$int));
   var responseDecoder = A2($Json$Decode._op[":="],"items",$Json$Decode.list(searchResultDecoder));
   var searchFeed = function (query) {
      var url = A2($Basics._op["++"],"https://api.github.com/search/repositories?q=",A2($Basics._op["++"],query,"+language:elm&sort=stars&order=desc"));
      var task = A2($Task.map,SetResults,A2($Http.get,responseDecoder,url));
      return A2($Task.onError,task,function (_p0) {    return $Task.succeed(SetResults(_U.list([])));});
   };
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "Search": return {ctor: "_Tuple2",_0: model,_1: $Effects.task(searchFeed(model.query))};
         case "SetQuery": return {ctor: "_Tuple2",_0: _U.update(model,{query: _p1._0}),_1: $Effects.none};
         case "SetResults": var newModel = _U.update(model,{results: _p1._0});
           return {ctor: "_Tuple2",_0: newModel,_1: $Effects.none};
         default: var newResults = A2($List.filter,function (_p2) {    var _p3 = _p2;return !_U.eq(_p3.id,_p1._0);},model.results);
           var newModel = _U.update(model,{results: newResults});
           return {ctor: "_Tuple2",_0: newModel,_1: $Effects.none};}
   });
   var app = $StartApp.start({view: view
                             ,update: update
                             ,init: {ctor: "_Tuple2",_0: initialModel,_1: $Effects.task(searchFeed(initialModel.query))}
                             ,inputs: _U.list([])});
